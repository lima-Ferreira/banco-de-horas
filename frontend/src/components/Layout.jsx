      ...
      >
        <div className="p-6 flex flex-col h-full">
          <div className="text-white font-black text-xl mb-10 flex justify-between items-center tracking-tighter">
            BANCO DE HORAS
            {/* Botão de fechar (apenas mobile) com o SVG CORRIGIDO */}
            <button onClick={fecharMenu} className="md:hidden text-white p-2">
              <svg
                width="24"
                height="24"
                viewBox="0 0 24 24"
          ...
          <nav className="flex-1 space-y-2">
            <Link
              to="/dashboard"
              onClick={fecharMenu}
              className={`flex items-center gap-3 p-4 rounded-xl font-bold text-sm transition-all ${isActive(
                "/dashboard"
              )}`}
            >
              Dashboard
        ...
        <header className="bg-white border-b border-gray-100 p-4 flex justify-between items-center sticky top-0 z-30">
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="md:hidden p-2 text-slate-600 active:scale-90 transition-all"
            >
              <svg
                width="24"
                height="24"
            ...
            <span className="font-black text-slate-800 md:hidden">
              OLÁ, LIMA
            </span>
          </div>
